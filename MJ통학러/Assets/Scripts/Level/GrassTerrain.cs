...
namespace Level
{
    public class GrassTerrain : MonoBehaviour, ITerrain
    {
        ...
        {
            var x = GetUniqueIntFromRange(-6, 7);
                        var possibilityArray = new []{false, false, false, true, false, false, true, true};
            ...
            var x2 = GetUniqueIntFromRange(-6, 7);
                        var possibilityArray2 = new []{false, true, false, true, false, false, true, true};
            //var possibilityArray = new []{false, false, false, false, false, false, false, true};
            var possibilityValue2 = RandomGenerator<bool>.RandomPicker(possibilityArray2);
            if (possibilityValue2)
            {
                var coinPossibility2 = new[] {false, true};
                var generateCoin2 = RandomGenerator<bool>.RandomPicker(coinPossibility2);
                if (generateCoin2)
                {
                    InstantiateObject(cuboid2, x, 2f, Quaternion.Euler(-90, 0, 0));

                }
                else
                {
                    var mob = RandomGenerator<GameObject>.RandomPicker(mobs.ToArray());
                    InstantiateObject(mob, x2, 1f, Quaternion.identity);
                }
            }
